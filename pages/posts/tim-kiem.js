...
function Home(props) {
    ...
    useEffect(() => {
        if (props.router.query.search) {
            getPostNormal(props.router.query.search);
        }
    ...
    const getPostNormal = async (valueSearch) => {
        setLoading(true);
                ...
                <Row style={{width: '100%', marginTop: 20}} gutter={[10, 10]}>
                    <Col xs={{span: 24}} md={{span: 15}}>
                        <div className='d-flex' style={{marginBottom: '20px'}}>
                            <strong style={{color: '#08979c', fontSize: '25px'}}>{t(`Tìm kiếm theo từ khóa ${titleProvince}`)} - </strong>&nbsp;
